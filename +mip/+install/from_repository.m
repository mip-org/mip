...
%   installedFqns - Cell array of FQNs newly installed by this call.
    ...
    % The hot swap only applies when the active root is the root the
    % running mip is installed into; in any other root (an activated
    % environment, an external MIP_ROOT target) gh/mip-org/core/mip is an
    % ordinary inert package and installs through the generic path.
...
% in place) and the graph build is retried.
    ...
    for attempt = 1:10
        ...
        allMissing = {};
        for i = 1:length(resolvedPackages)
            [installOrder, missing] = mip.dependency.build_graph(resolvedPackages{i}.fqn, packageInfoMap);
    ...
    % Sort topologically. build_graph already returns each
